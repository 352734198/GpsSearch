...
public abstract class Chart<T extends ChartData<? extends IDataSet<? extends Entry>>> extends
    ...
    protected void onDraw(Canvas canvas) {
        // super.onDraw(canvas);
        System.out.println("----------------->>>> mNoDataText =" + mNoDataText);
        System.out.println("----------------->>>> mNoDataTextDescription =" + mNoDataTextDescription);
        if (mData == null) {
            ...
            System.out.println("------------->>> mdate == null");
            return;
        }
        System.out.println("------------->>> mdate != null");
        if (!mOffsetsCalculated) {
            System.out.println("------------------->>>>>> !mOffsetsCalculated");
            calculateOffsets();
        ...
        for (int i = 0; i < mIndicesToHighlight.length; i++) {
            ...
            if (xIndex <= deltaX && xIndex <= deltaX * mAnimator.getPhaseX()) {
               ...
               // mMarkerView.refreshContent(e, highlight);
                mMarkerView.refreshContent(e,highlight,getXValue(xIndex));
